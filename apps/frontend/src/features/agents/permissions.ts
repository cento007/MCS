 ...
 * twelve switches from that list. It would also be the most dangerous thing this screen could do.
 * A switch reading `Merge: off` that gates nothing is worse than no switch at all: an operator
 * reads it as a guarantee and stops watching. This codebase already shipped one setting nothing
 * reads (`integrations.ollama.enabled`); that one wasted a configuration step, and this one would
 * be a safety claim.
 *
 ...
 *    has no path to them.
 *
 * This client renders the three and **says the nine are absent, on the record**, rather than
 * leaving an operator who has read the PRD to wonder whether the screen forgot them.
 *
 ...
 * claim this screen makes from a spec: it is a list the API produced, in the runtime's own
 ...
 * widen what the operator's own Claude Code settings already allow, and the Builder says so —
 * because "grant" read as "authorise" is the single most likely misreading of this screen.
 */
...
/** Every PRD §5.5 permission the Backend does not model, so the screen can say so out loud. */
 ...
 * *narrower* agent on this screen in exactly the way it does on the server, instead of the two
