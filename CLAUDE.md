...
**Workflows (PRD §5.6) are not built, and `POST /agents/{id}/executions` with them.** A workflow is an ordered chain of executions and no execution primitive exists; `agent.execution_*` remain reserved and unproduced. Membership carries **no ordinal and no role**: a team is a set, the order will belong to the workflow when one exists, and §5.7's role names are the agents' own names.
