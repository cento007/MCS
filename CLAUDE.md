...
Known gaps: `openapi.yaml` is emitted but declares **no response schemas** (`x-mc-response-schema: undeclared` on every operation) and types `error.code` as a pattern rather than an enum, so `apps/frontend/src/lib/api/types.ts` is still hand-written and response types are not yet derivable from the spec; `sessions.runtime` still admits `'ollama'` though nothing can launch such a session (`ManagedRuntime` always drives the Claude SDK), so that CHECK is the dishonest one and belongs to a multi-runtime slice; `GET /schedule` has no `memory_retention` row; and `integrations.ollama.enabled` is still read by nothing. **Graphify (PRD §6.2.1) is untouched and explicitly optional** — its adoption path starts with a hands-on trial as a per-repository Claude Code skill.
