...
Three contract items remain. All are WS2's, all are leaf features, none blocks Phase 1 work starting.

| # | Item | Why it is open | Specification |
|---|---|---|---|
| **1** | **No spend/usage aggregate endpoint** | Accepted UX finding **WC1** puts a spend-vs-budget stat on the Dashboard and a `$x.xx / $y.yy` chip in the top bar, and PRD §4.4.2 mandates cost-budget alerts. `Session.costUsd` is per-session and lists are cursor-paginated, so a client cannot compute "today's spend" without walking every page. Found at this gate. | Add `GET /api/v1/spend` → `{ periodStart, periodEnd, totalCostUsd, sessionCount, budget: { dailyUsd, perSessionUsd } }`, period defaulting to the current day in `general.timezone`. One `sum(total_cost_usd)` over `sessions`; **no WS3 change** (column and date-ranged index already exist). |
| **2** | **Session-scoped Files (and Commits) panel has no endpoint** | WS4 §6.7 specifies right-panel tabs Commits / Files / Timeline / Notes with query keys `['sessions', id, …]`, and PRD §8.3 names Files explicitly. `timeline` exists; commits are reachable only via `/repositories/{id}/commits?sessionId=`; **Files has no source at all** — `files[]` is returned only on single-commit fetches, so the panel would need N+1 requests. Found at this gate. | Add `GET /api/v1/sessions/{id}/files` → aggregated union across the Session's commits: `{ path, status, additions, deletions, commitCount }`. Add `GET /api/v1/sessions/{id}/commits` as a nested convenience list (same pattern as `/repositories/{id}/pull-requests`). No WS3 change — both derive from `commits` + `commits.files`. |
...
Also outstanding, and cheap: **A13** (session-title auto-derivation) needs one paragraph in WS2 §6.4/§6.8 stating the Backend-side rule; WS1 implements it. The behaviour is fully specified in §5/A13 — only its transcription into the API contract is pending.
...
| N18 | **WS5 §5.6 links the sync-conflict count to `sync.failed`**, but conflicts are `sync.conflict_detected` (catalog #26). | Cosmetic wireframe annotation; name the correct event. |
| N19 | **WS5 §6.2 renders "Runtime → Agent → Task" as sample UI copy**, introducing "Task" where F4.1 has no such entity. | Verbatim PRD §5.1 copy inside a correctly-marked Phase 4 placeholder shell — acceptable, but worth a footnote given "Upcoming Tasks" also appears in the same document. |
| N20 | **ADR status is never rendered in WS5's wireframes** (list shows ID/Title/Project/Date; detail shows the four template sections), so the arbitrated `proposed\|accepted\|rejected\|superseded` set has no visual surface. | Add a status chip to the ADR list and detail when Phase 2 UI is built; the vocabulary itself is settled (A4). |
