...
| **D11** | **Ollama is not offered as an agent runtime, and the two settings keys that said it was are withdrawn.** `PUT /settings/integrations/ollama` is `{ host, port }`; `integrations.ollama.enabled` and `integrations.ollama.defaultModel` no longer exist, and `ck_sessions_runtime` admits only `claude_code`. *(2026-08-14)* | PRD §5.4 lists Ollama as an optional V1 agent runtime; WS2 §7.2 specified the four-field `ollama` document; WS3 §3.9 specified `runtime IN ('claude_code','ollama')`. | **Nothing read either key, in any phase.** `AGENT_RUNTIMES` has one member because one runtime is launchable (F1.5) — the managed runtime drives the Claude Agent SDK unconditionally — and Phase 4 slice 1 declined `agents.defaultRuntime` for the same reason. `enabled` had become this codebase's standing example of a setting nothing reads, cited in six modules; keeping it while citing it was the contradiction. It could not be honestly repurposed as "is Ollama in use" either: that is already answered by `integrations.qdrant.embeddingModel`, and a second answer to one question is the same defect in a new costume. **Ollama itself is untouched and still required for Phase 3** — `host`/`port` are read on every embedding call. Widening again is one edit to `AGENT_RUNTIMES` plus a CHECK alter on `agents` and `sessions`, which the schema now derives from that one array. | Migrations `0010`, `0011`; `packages/shared/src/settings/registry.ts`; `packages/shared/src/db/schema/sessions.ts`; WS2 §7.2, §7.3; WS3 §3.9. |
