...
CREATE TABLE sessions (
  ...
  runtime                  text NOT NULL DEFAULT 'claude_code'
                           CHECK (runtime IN ('claude_code', 'ollama')),   -- V2 runtimes via CHECK alter
...
- **Table inventory:** 20 Phase 1–2 tables (`users`, `auth_sessions`, `api_tokens`, `workspaces`, `projects`, `repositories`, `commits`, `pull_requests`, `sessions`, `session_events`, `messages`, `transcript_tail_states`, `adrs`, `notifications`, `settings`, `secret_items`, `audit_log_entries`, `obsidian_sync_states`, `service_heartbeats`, `sync_runs`) + 3 Phase 3/4 skeletons (`memory_items`, `agents`, `agent_teams`) + vendored `pgboss` schema. Full-text search (§4.6) adds no table — five generated columns and five GIN indexes on existing ones.
